...
int main() {
    ...
    while (!kidName.empty())
    {
        ...
        do {
            ...
            aNiceList.addGift(giftName);
        } while(!giftName.empty());

        theNiceList.push_back(aNiceList);     // push_back inserts elements at end in list
    ...
    for (auto oneKidList: theNiceList)
        std::cout << oneKidList.getName() << "  " << oneKidList.giftsToString(" ") << std::endl;
